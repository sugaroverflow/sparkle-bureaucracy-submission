...
:root{
  ...
  --measure:clamp(34rem,50vw,48rem); --col:58rem; --pad:clamp(1rem,4vw,2.5rem); --wrap:96rem;
...
.permit{position:relative;border:2px solid var(--ink);background:#fff;box-shadow:8px 8px 0 var(--magenta);
  max-width:68rem;overflow:hidden;margin-top:1.8rem}
...
.vplay{position:absolute;bottom:calc(1.3rem + 50px);left:1.6rem;width:4.4rem;height:4.4rem;border-radius:50%;
  background:var(--foil);background-size:200%;border:2px solid var(--ink);box-shadow:3px 3px 0 var(--ink);
  display:grid;place-content:center;transform:rotate(-4deg);transition:transform .18s}
...
.vfacade:hover .vplay,.vfacade:focus-visible .vplay{transform:scale(1.1) rotate(5deg)}
...
.reading.wide-ok>.tbl{max-width:var(--col)}
...
/* ── thank-you cards ─────────────────────────────────────── */
#part-08 .bullets{columns:1;column-gap:1rem;max-width:var(--col)}
...
.shots.gallery img{aspect-ratio:auto;height:100%;width:100%;object-fit:cover;object-position:50% 40%}
  ...
  <div class="permit">
    ...
    <div class="permit-grid">
      <div>
        ...
        <div class="window">
          ${VIDEO_EMBED
    ? `<button class="vfacade" id="vfacade" data-embed="${VIDEO_EMBED}" aria-label="Play the official introduction video"><img src="/assets/video-cover.jpg" alt="Official introduction — video cover"><span class="vplay" aria-hidden="true"></span></button>`
...
(function(){
  ...
  var vf=document.getElementById('vfacade');
  if(vf)vf.addEventListener('click',function(){
    var f=document.createElement('iframe');
    f.src=vf.getAttribute('data-embed')+'?autoplay=1';
    f.title='Video introduction';
